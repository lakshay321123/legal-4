...
export async function POST(req: Request) {
  try {
    ...
    const system =
      mode === 'lawyer'
        ? 'You are a legal research assistant. Be precise and concise.'
        : 'You are a helpful legal explainer. Use simple language.';

    // Use fetch to avoid SDK/version issues
    ...
    const answer =
      data?.choices?.[0]?.message?.content ?? 'No answer generated by the model.';
